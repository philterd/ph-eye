...
import models.french_medical as module

ENTITY = {"label": "Maladie", "score": 0.75, "text": "diabète", "start": 12, "end": 19}
...
def test_predict_returns_entity():
    ...
    result = module.predict(model, "Il souffre de diabète.", ["Maladie"], 0.0)
    assert len(result) == 1
    assert result[0]["label"] == "Maladie"
    assert result[0]["text"] == "diabète"
...
def test_predict_score_is_float():
    model = MagicMock()
    model.predict_entities.return_value = [ENTITY]
    result = module.predict(model, "text", ["Maladie"], 0.0)
    assert isinstance(result[0]["score"], float)


def test_predict_passes_threshold_and_flat_ner_to_model():
    model = MagicMock()
    model.predict_entities.return_value = []
    module.predict(model, "text", ["Maladie"], 0.3)
    model.predict_entities.assert_called_once_with("text", ["Maladie"], threshold=0.3, flat_ner=True)


def test_predict_returns_empty_list_when_no_entities():
    model = MagicMock()
    model.predict_entities.return_value = []
    result = module.predict(model, "text", ["Maladie"], 0.3)
    assert result == []
